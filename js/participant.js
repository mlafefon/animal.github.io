...
const timerDisplay = document.getElementById('participant-timer-display');
...
function startParticipantTimer(startTime) {
    stopParticipantTimer(); // Ensure any old one is cleared
    ...
    participantTimerInterval = setInterval(() => {
        ...
        if (timeLeft < 0) {
            // The timer on the participant side is just a visual aid.
            // The host's state change is the source of truth for when time is up.
            // So, we just stop our local timer.
...
function updateGameView(state) {
    ...
    // If the state is 'question' and we have timer data, start the timer.
    // For all other states, stop it. This is the source of truth.
    if (state.gameState === 'question' && state.currentQuestionData && typeof state.currentQuestionData.timer !== 'undefined') {
